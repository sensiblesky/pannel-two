    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8" x-data="{ showCreateModal: false, editItem: null }">
        ...
        <div class="card mt-3">
            ...
            <div class="flex flex-col space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4 sm:px-5">
                <form method="GET" action="{{ route('agent.tickets/settings-canned-responses') }}" class="flex flex-1 flex-wrap items-center gap-3">
                    ...
                    @if(request()->hasAny(['search', 'category']))
                        <a href="{{ route('agent.tickets/settings-canned-responses') }}" class="btn bg-slate-150 font-medium text-slate-800 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200/80 dark:bg-navy-500 dark:text-navy-50 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">Clear</a>
                    @endif
                </form>
            </div>
                            ...
                            <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="flex space-x-2">
                                        ...
                                        <form id="delete-form-{{ $resp->id }}" action="{{ route('agent.tickets/settings-canned-responses-destroy', $resp->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 text-center">
                                    <div class="flex flex-col items-center space-y-2">
                                        ...
                                        @if(request()->hasAny(['search', 'category']))
                                            <a href="{{ route('agent.tickets/settings-canned-responses') }}" class="text-sm text-primary hover:text-primary-focus dark:text-accent-light dark:hover:text-accent">Clear filters</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
        ...
        <template x-teleport="body">
            <div x-show="showCreateModal" x-transition.opacity class="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/60" @click.self="showCreateModal = false">
                <div class="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-navy-700" @click.stop>
                    ...
                    {{-- Create Form --}}
                    <form x-show="!editItem" method="POST" action="{{ route('agent.tickets/settings-canned-responses-store') }}" class="mt-4 space-y-4">
                        @csrf
                        <label class="block">
                    ...
                    <template x-if="editItem">
                        <form method="POST" :action="`{{ url('/agent/tickets/settings/canned-responses') }}/${editItem.id}`" class="mt-4 space-y-4">
                            @csrf @method('PUT')
                            <label class="block">
                                <span class="font-medium text-slate-600 dark:text-navy-100">Title <span class="text-error">*</span></span>
