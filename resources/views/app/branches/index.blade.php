<x-app-layout title="Branches / Campus" is-sidebar-open="true" is-header-blur="true">
    @slot('script')
    <script>
        function confirmDelete(branchUuid, branchName) {
            Swal.fire({
                title: 'Delete Branch?',
                html: `<p class="text-slate-500">Are you sure you want to delete <strong>${branchName}</strong>?<br>This action cannot be undone.</p>`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    document.getElementById('delete-branch-' + branchUuid).submit();
                }
            });
        }
            ...
            <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                <li class="flex items-center space-x-2">
                    <a class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent"
                        href="{{ route('config/branches') }}">Configuration</a>
        ...
        <div class="grid grid-cols-1 gap-4 sm:gap-5 lg:gap-6">
            <div>
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('config/branches-create') }}"
                ...
                <div class="card mt-3">
                    <div class="flex flex-col space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4 sm:px-5">
                        <form method="GET" action="{{ route('config/branches') }}" class="flex flex-1 flex-wrap items-center gap-3">
                            ...
                            <tbody>
                                @forelse ($branches as $branch)
                                    <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">{{ $branches->firstItem() + $loop->index }}</td>
                                        <td class="whitespace-nowrap px-3 py-3 font-medium text-slate-700 dark:text-navy-100 lg:px-5">
                                            <div class="flex items-center space-x-3">
                                                <div class="avatar flex size-10">
                                                    <div class="is-initial rounded-full bg-info/10 text-info dark:bg-info/15">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                            {{ collect([$branch->city, $branch->country])->filter()->implode(', ') ?: '-' }}
                                        </td>
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                            ...
                                            <div x-data="usePopper({placement:'bottom-end',offset:4})"
                                                ...
                                                <div x-ref="popperRoot" class="popper-root" :class="isShowPopper && 'show'">
                                                    <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                                        <ul>
                                                            <li>
                                                                <a href="{{ route('config/branches-edit', $branch) }}"
                                                            ...
                                                            <li>
                                                                <form id="delete-branch-{{ $branch->uuid }}" method="POST" action="{{ route('config/branches-destroy', $branch) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="button"
                                                                        onclick="confirmDelete('{{ $branch->uuid }}', '{{ e($branch->name) }}')"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="px-4 py-6 text-center text-slate-400 dark:text-navy-300">No branches found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                        <div class="flex items-center space-x-2 text-xs-plus">
                            <span>Showing</span>
                            <span class="font-semibold">{{ $branches->firstItem() ?? 0 }} - {{ $branches->lastItem() ?? 0 }}</span>
                            <span>of {{ $branches->total() }} entries</span>
                        </div>
                        @if ($branches->hasPages())
                            <ol class="pagination">
                                <li class="rounded-l-lg bg-slate-150 dark:bg-navy-500">
                                    ...
                                    @else
                                        <a href="{{ $branches->previousPageUrl() }}" class="flex size-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-300 focus:bg-slate-300 active:bg-slate-300/80 dark:text-navy-200 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" /></svg>
                                        </a>
                                    @endif
                                </li>
                                @foreach ($branches->getUrlRange(1, $branches->lastPage()) as $page => $url)
                                    <li class="bg-slate-150 dark:bg-navy-500">
                                        @if ($page == $branches->currentPage())
                                            <span class="flex h-8 min-w-[2rem] items-center justify-center rounded-lg bg-primary px-3 leading-tight text-white transition-colors dark:bg-accent">{{ $page }}</span>
                                        @else
                                ...
                                <li class="rounded-r-lg bg-slate-150 dark:bg-navy-500">
                                    @if ($branches->hasMorePages())
                                        <a href="{{ $branches->nextPageUrl() }}" class="flex size-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-300 focus:bg-slate-300 active:bg-slate-300/80 dark:text-navy-200 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
