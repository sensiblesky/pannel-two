<x-app-layout title="Customers Management" is-sidebar-open="true" is-header-blur="true">
    @slot('script')
    <script>
        function confirmDelete(customerUuid, customerName) {
            Swal.fire({
                title: 'Delete Customer?',
                html: `<p class="text-slate-500">Are you sure you want to delete <strong>${customerName}</strong>?<br>This action cannot be undone.</p>`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    document.getElementById('delete-customer-' + customerUuid).submit();
                }
            });
        }
            ...
            <div>
                ...
                <div class="card mt-3">
                    ...
                    <div class="flex flex-col space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4 sm:px-5">
                        <form method="GET" action="{{ route('users/customers') }}" class="flex flex-1 flex-wrap items-center gap-3">
                            ...
                            <label class="block">
                                <select name="source"
                                    ...
                                    <option value="widget" {{ request('source') === 'widget' ? 'selected' : '' }}>Widget</option>
                                    <option value="ticket" {{ request('source') === 'ticket' ? 'selected' : '' }}>Ticket</option>
                            ...
                            <tbody>
                                @forelse ($customers as $customer)
                                    <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                            @php
                                                $sourceBg = match($customer->source) {
                                                    'widget' => 'bg-info/10 text-info dark:bg-info/15',
                                                    'ticket' => 'bg-warning/10 text-warning dark:bg-warning/15',
                                                    'import' => 'bg-secondary/10 text-secondary dark:bg-secondary/15',
                                                    'api' => 'bg-primary/10 text-primary dark:bg-accent-light/15 dark:text-accent-light',
                                                    'manual' => 'bg-slate-200 text-slate-600 dark:bg-navy-500 dark:text-navy-100',
                                                    default => 'bg-slate-200 text-slate-600',
                                                };
                                            @endphp
                                            <div class="badge rounded-full {{ $sourceBg }}">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                            @php
                                                $statusBg = match($customer->status) {
                                                    ...
                                                    'blocked' => 'bg-warning/10 text-warning dark:bg-warning/15',
                                                    'deleted' => 'bg-error/10 text-error dark:bg-error/15',
                                                    default => 'bg-slate-200 text-slate-600',
                                                };
                                            @endphp
                                            <div class="badge rounded-full {{ $statusBg }}">
                                                {{ ucfirst($customer->status) }}
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                            {{ $customer->last_seen_at?->diffForHumans() ?? 'Never' }}
                                        </td>
                                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                            <div x-data="usePopper({placement:'bottom-end',offset:4})"
                                                ...
                                                <div x-ref="popperRoot" class="popper-root"
                                                    ...
                                                    <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                                        <ul>
                                                            <li>
                                                                <a href="{{ route('users/customers-edit', $customer) }}"
                                                            ...
                                                            <li>
                                                                <form id="delete-customer-{{ $customer->uuid }}" method="POST" action="{{ route('users/customers-destroy', $customer) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="button"
                                                                        onclick="confirmDelete('{{ $customer->uuid }}', '{{ e($customer->full_name) }}')"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="px-4 py-6 text-center text-slate-400 dark:text-navy-300">
                                            No customers found.
                                        </td>
                                    </tr>
                                @endforelse
                    ...
                    <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                        <div class="flex items-center space-x-2 text-xs-plus">
                            <span>Showing</span>
                            <span class="font-semibold">{{ $customers->firstItem() ?? 0 }} - {{ $customers->lastItem() ?? 0 }}</span>
                            <span>of {{ $customers->total() }} entries</span>
                        </div>
