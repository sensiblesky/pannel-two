<x-app-layout title="Edit Department" is-sidebar-open="true" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
            <h2 class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl">Edit Department</h2>
            <div class="hidden h-full py-1 sm:flex"><div class="h-full w-px bg-slate-300 dark:bg-navy-600"></div></div>
            <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                <li class="flex items-center space-x-2">
                    <a class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent" href="{{ route('config/departments') }}">Departments</a>
            ...
            <div>
                <div class="card">
                    <div class="border-b border-slate-200 p-4 dark:border-navy-500 sm:px-5">
                        <h2 class="text-lg font-medium tracking-wide text-slate-700 dark:text-navy-100">Edit: {{ $department->name }}</h2>
                    </div>
                    <form method="POST" action="{{ route('config/departments-update', $department) }}" class="space-y-4 p-4 sm:p-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <label class="block">
                                ...
                                <input class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent @error('name') border-error @enderror"
                                    placeholder="e.g. Human Resources" type="text" name="name" value="{{ old('name', $department->name) }}" />
                                @error('name')<span class="text-tiny-plus text-error">{{ $message }}</span>@enderror
                            </label>
                            <label class="block">
                                <span class="font-medium text-slate-600 dark:text-navy-100">Department Code</span>
                                <input class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent @error('code') border-error @enderror"
                                    placeholder="e.g. HR-001" type="text" name="code" value="{{ old('code', $department->code) }}" />
                        ...
                        <label class="block">
                            ...
                            <textarea name="description" rows="2"
                                ...
                                placeholder="Brief description (optional)">{{ old('description', $department->description) }}</textarea>
                        </label>
                            ...
                            <label class="block">
                                ...
                                <select name="branch_id"
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch_id', $department->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            </label>
                            <label class="block">
                                ...
                                <select name="status"
                                    class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
                                    <option value="1" {{ old('status', (string)$department->status) === '1' ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('status', (string)$department->status) === '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </label>
                        </div>
