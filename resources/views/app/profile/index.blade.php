        ...
        <div class="flex items-center space-x-4 py-5 lg:py-6">
            ...
            <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                <li class="flex items-center space-x-2">
                    <a class="text-primary transition-colors hover:text-primary-focus dark:text-accent-light dark:hover:text-accent" href="{{ route('profile') }}">Profile</a>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="card p-4 sm:p-5">
                    ...
                    <ul class="mt-6 space-y-1.5 font-inter font-medium">
                        <li>
                            <a @click.prevent="activeTab = 'account'" href="{{ route('profile') }}"
                        ...
                        <li>
                            <a @click.prevent="activeTab = 'security'" href="{{ route('profile', ['tab' => 'security']) }}"
                        ...
                        <li>
                            <a @click.prevent="activeTab = 'sessions'" href="{{ route('profile', ['tab' => 'sessions']) }}"
                                    ...
                                    @unless ($user->email_verified_at)
                                        <div class="mt-2 flex items-center space-x-2" x-data="{
                                            ...
                                            async sendCode() {
                                                ...
                                                this.sending = true;
                                                try {
                                                    const res = await fetch('{{ route('profile/send-verification') }}', {
                                                        method: 'POST',
                                                        headers: {
                                            ...
                                            <template x-teleport="#x-teleport-target">
                                                <div class="fixed inset-0 z-100 flex flex-col items-center justify-center overflow-hidden px-4 py-6 sm:px-5"
                                                    ...
                                                    <div class="relative w-full max-w-md rounded-lg bg-white transition-opacity duration-300 dark:bg-navy-700 overflow-y-auto"
                                                        ...
                                                        <form method="POST" action="{{ route('profile/verify-email') }}" class="px-4 py-4 sm:px-5">
                                                            @csrf
                ...
                <div class="space-y-4 sm:space-y-5 lg:space-y-6">
                    ...
                    <div class="card" x-data="{
                        ...
                        async setupAuthApp() {
                            this.loading = true;
                            try {
                                const res = await fetch('{{ route('profile/setup-2fa') }}', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        ...
                        async sendEmailCode(password) {
                            ...
                            this.emailError = '';
                            try {
                                const res = await fetch('{{ route('profile/send-email-2fa-code') }}', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            ...
                            <div class="fixed inset-0 z-100 flex flex-col items-center justify-center overflow-hidden px-4 py-6 sm:px-5"
                                ...
                                <div class="relative w-full max-w-lg rounded-lg bg-white transition-opacity duration-300 dark:bg-navy-700 overflow-y-auto"
                                    ...
                                    <div class="px-4 py-4 sm:px-5" x-show="step === 'scan'" x-cloak>
                                        ...
                                        <form method="POST" action="{{ route('profile/confirm-2fa') }}" class="mt-5">
                                            @csrf
                                            <label class="block">
                                    ...
                                    <div class="px-4 py-4 sm:px-5" x-show="step === 'email-verify'" x-cloak>
                                        ...
                                        <form method="POST" action="{{ route('profile/confirm-email-2fa') }}" class="mt-5">
                                            @csrf
                                            <label class="block">
                        ...
                        <template x-teleport="#x-teleport-target">
                            <div class="fixed inset-0 z-100 flex flex-col items-center justify-center overflow-hidden px-4 py-6 sm:px-5"
                                ...
                                <div class="relative w-full max-w-lg rounded-lg bg-white transition-opacity duration-300 dark:bg-navy-700 overflow-y-auto"
                                    ...
                                    {{-- Modal Body --}}
                                    <form method="POST" action="{{ route('profile/disable-2fa') }}" class="px-4 py-4 sm:px-5">
                                        @csrf
                                        @method('DELETE')
                                        <div class="flex items-center space-x-3 rounded-lg bg-warning/10 px-4 py-3 text-warning">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    ...
                                    @foreach ($sessions as $session)
                                        <div class="flex items-center justify-between rounded-lg border border-slate-200 p-4 dark:border-navy-500 {{ $session->is_current ? 'bg-primary/5 border-primary/30 dark:bg-accent/5 dark:border-accent/30' : '' }}">
                                            ...
                                            @unless ($session->is_current)
                                                <form method="POST" action="{{ route('profile/destroy-session', $session->id) }}"
                        ...
                        <template x-teleport="#x-teleport-target">
                            <div class="fixed inset-0 z-100 flex flex-col items-center justify-center overflow-hidden px-4 py-6 sm:px-5"
                                ...
                                <div class="relative w-full max-w-md rounded-lg bg-white transition-opacity duration-300 dark:bg-navy-700 overflow-y-auto"
                                    ...
                                    <form method="POST" action="{{ route('profile/destroy-other-sessions') }}" class="px-4 py-4 sm:px-5">
                                        @csrf
                                        @method('DELETE')
                                        <div class="text-center">
