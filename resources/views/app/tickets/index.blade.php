<x-app-layout title="All Tickets" is-sidebar-open="true" is-header-blur="true">
        ...
        <script>
            ...
            function ticketPoller() {
                return {
                    ...
                    interval: null,
                    start() {
                        // Load the configured alert sound
                    ...
                    async poll() {
                        try {
                            const res = await fetch(`{{ route("tickets/poll") }}?since=${encodeURIComponent(this.since)}`);
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="flex items-center justify-between py-5 lg:py-6">
            ...
            <a href="{{ route('tickets/create') }}" class="btn space-x-2 bg-primary font-medium text-white shadow-lg shadow-primary/50 hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:shadow-accent/50 dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
        ...
        <div class="is-scrollbar-hidden mb-4 overflow-x-auto rounded-lg bg-slate-100 dark:bg-navy-800">
            <div class="flex">
                ...
                @foreach($tabs as $key => $label)
                    <a href="{{ route('tickets/index', ['view' => $key]) }}"
        ...
        <div x-data="{ selectedTickets: [], showBulk: false }" x-init="$watch('selectedTickets', value => showBulk = value.length > 0)">
            <div class="card mt-3">
                ...
                <div x-show="showBulk" x-collapse class="flex items-center justify-between border-b border-slate-200 bg-primary/5 px-4 py-3 dark:border-navy-500 dark:bg-accent/5">
                    ...
                    <div class="flex items-center space-x-2">
                        <form method="POST" action="{{ route('tickets/bulk-action') }}" class="flex items-center space-x-2">
                            @csrf
                            ...
                            <div x-data="{
                                    ...
                                    async searchAgent() {
                                        ...
                                        try {
                                            const r = await fetch(`{{ route('tickets/search-agents') }}?q=${encodeURIComponent(this.agentQuery)}`, { headers: {'Accept':'application/json','X-Requested-With':'XMLHttpRequest'} });
                ...
                <div class="flex flex-col space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 sm:px-5">
                    <form method="GET" action="{{ route('tickets/index') }}" class="flex flex-1 flex-col space-y-3 sm:flex-row sm:items-center sm:space-x-3 sm:space-y-0">
                        ...
                        <div x-data="{
                                searchUrl: '{{ route('tickets/search-agents') }}',
                        ...
                        <a href="{{ route('tickets/index', ['view' => $view]) }}" class="btn border border-slate-300 font-medium text-slate-800 hover:bg-slate-150 dark:border-navy-450 dark:text-navy-50 dark:hover:bg-navy-500">Clear</a>
                    </form>
                </div>
                        ...
                        <tbody>
                            @forelse($tickets as $ticket)
                                <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                        <a href="{{ route('tickets/show', $ticket->uuid) }}" class="font-medium text-primary hover:text-primary-focus dark:text-accent-light dark:hover:text-accent">{{ $ticket->ticket_no }}</a>
                                    </td>
                                    <td class="px-4 py-3 sm:px-5">
                                        <a href="{{ route('tickets/show', $ticket->uuid) }}" class="max-w-[250px] truncate block text-sm font-medium text-slate-700 hover:text-primary dark:text-navy-100 dark:hover:text-accent-light">{{ $ticket->subject }}</a>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                        <div x-data="usePopper({placement:'bottom-end',offset:4})" @click.outside="if(isShowPopper) isShowPopper = false" class="inline-flex">
                                            ...
                                            <div x-ref="popperRoot" class="popper-root" :class="isShowPopper && 'show'">
                                                <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                                    <ul>
                                                        <li>
                                                            <a href="{{ route('tickets/show', $ticket->uuid) }}" class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide text-slate-600 outline-hidden transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:text-navy-100 dark:hover:bg-navy-600 dark:hover:text-navy-50 dark:focus:bg-navy-600 dark:focus:text-navy-50">
                                                                View
                                                            </a>
                                                        </li>
                                                        <li>
                                        ...
                                        <form id="delete-form-{{ $ticket->uuid }}" action="{{ route('tickets/destroy', $ticket->uuid) }}" method="POST" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="9" class="px-4 py-8 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <a href="{{ route('tickets/create') }}" class="btn mt-4 bg-primary text-sm font-medium text-white hover:bg-primary-focus dark:bg-accent dark:hover:bg-accent-focus">Create First Ticket</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
