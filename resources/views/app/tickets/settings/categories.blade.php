<x-app-layout title="Ticket Categories" is-sidebar-open="true" is-header-blur="true">
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8" x-data="{ showCreateModal: false, editItem: null }">
        <div class="flex items-center justify-between py-5 lg:py-6">
            <div class="flex items-center space-x-4">
                <h2 class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl">Categories</h2>
        ...
        <div class="card mt-3">
            ...
            <div class="flex flex-col space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4 sm:px-5">
                <form method="GET" action="{{ route('tickets/settings-categories') }}" class="flex flex-1 flex-wrap items-center gap-3">
                    ...
                    @if(request()->hasAny(['search', 'status']))
                        <a href="{{ route('tickets/settings-categories') }}"
                            class="btn bg-slate-150 font-medium text-slate-800 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200/80 dark:bg-navy-500 dark:text-navy-50 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
                            Clear
                        </a>
                    @endif
                ...
                <table class="is-hoverable w-full text-left">
                    <thead>
                        <tr>
                            ...
                            <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Description</th>
                    ...
                    <tbody>
                        @forelse($categories as $cat)
                            <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-500 dark:text-navy-200 sm:px-5">{{ $categories->firstItem() + $loop->index }}</td>
                                <td class="whitespace-nowrap px-4 py-3 font-medium text-slate-700 dark:text-navy-100 sm:px-5">{{ $cat->name }}</td>
                                <td class="max-w-xs truncate px-4 py-3 text-sm text-slate-500 dark:text-navy-200 sm:px-5">{{ $cat->description ?? '—' }}</td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="badge rounded-full {{ $cat->status ? 'bg-success/10 text-success dark:bg-success/15' : 'bg-slate-200 text-slate-500 dark:bg-navy-500 dark:text-navy-200' }}">{{ $cat->status ? 'Active' : 'Inactive' }}</div>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-500 dark:text-navy-200 sm:px-5">{{ \Carbon\Carbon::parse($cat->created_at)->format('M d, Y') }}</td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="flex space-x-2">
                                        <button @click="showCreateModal = true; editItem = { id: {{ $cat->id }}, name: '{{ addslashes($cat->name) }}', description: '{{ addslashes($cat->description ?? '') }}', status: {{ $cat->status ? 'true' : 'false' }} }"
                                        ...
                                        <button @click="confirmDelete({{ $cat->id }})" class="btn size-8 rounded-full p-0 hover:bg-error/20 focus:bg-error/20 active:bg-error/25 text-error">
                                        ...
                                        <form id="delete-form-{{ $cat->id }}" action="{{ route('tickets/settings-categories-destroy', $cat->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center">
                                    <div class="flex flex-col items-center space-y-2">
                                        ...
                                        <p class="text-slate-400 dark:text-navy-300">No categories found</p>
                                        @if(request()->hasAny(['search', 'status']))
                                            <a href="{{ route('tickets/settings-categories') }}" class="text-sm text-primary hover:text-primary-focus dark:text-accent-light dark:hover:text-accent">Clear filters</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                <div class="flex items-center space-x-2 text-xs-plus">
                    <span>Showing</span>
                    <span class="font-semibold">{{ $categories->firstItem() ?? 0 }} - {{ $categories->lastItem() ?? 0 }}</span>
                    <span>of {{ $categories->total() }} entries</span>
                </div>
                ...
                @if ($categories->hasPages())
                    <ol class="pagination">
                        <li class="rounded-l-lg bg-slate-150 dark:bg-navy-500">
                            ...
                            @else
                                <a href="{{ $categories->previousPageUrl() }}" class="flex size-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-300 focus:bg-slate-300 active:bg-slate-300/80 dark:text-navy-200 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" /></svg>
                                </a>
                            @endif
                        </li>
                        ...
                        <li class="rounded-r-lg bg-slate-150 dark:bg-navy-500">
                            @if ($categories->hasMorePages())
                                <a href="{{ $categories->nextPageUrl() }}" class="flex size-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-300 focus:bg-slate-300 active:bg-slate-300/80 dark:text-navy-200 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
        ...
        <template x-teleport="body">
            <div x-show="showCreateModal" x-transition.opacity class="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/60" @click.self="showCreateModal = false">
                <div class="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-navy-700" @click.stop>
                    <h3 class="text-lg font-medium text-slate-700 dark:text-navy-100" x-text="editItem ? 'Edit Category' : 'Add Category'"></h3>

                    {{-- Create Form --}}
                    <form x-show="!editItem" method="POST" action="{{ route('tickets/settings-categories-store') }}" class="mt-4 space-y-4">
                        @csrf
                        <label class="block">
                            <span class="font-medium text-slate-600 dark:text-navy-100">Name</span>
                            <input class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" type="text" name="name" required>
                        </label>
                        <label class="block">
                            <span class="font-medium text-slate-600 dark:text-navy-100">Description</span>
                            <textarea class="form-textarea mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" rows="2" name="description"></textarea>
                        </label>
                    ...
                    <template x-if="editItem">
                        <form method="POST" :action="`/tickets/settings/categories/${editItem.id}`" class="mt-4 space-y-4">
                            @csrf @method('PUT')
                            <label class="block">
                                <span class="font-medium text-slate-600 dark:text-navy-100">Name</span>
                                <input class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" type="text" name="name" x-model="editItem.name" required>
                            </label>
                            <label class="block">
                                <span class="font-medium text-slate-600 dark:text-navy-100">Description</span>
                                <textarea class="form-textarea mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" rows="2" name="description" x-model="editItem.description"></textarea>
                            </label>
                            <label class="inline-flex items-center space-x-2">
