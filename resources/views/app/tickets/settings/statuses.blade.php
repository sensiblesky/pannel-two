<x-app-layout title="Ticket Statuses" is-sidebar-open="true" is-header-blur="true">

    @slot('script')
        <script>
            function confirmDelete(id) {
                Swal.fire({ title: 'Are you sure?', text: 'This status will be deleted.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#e53e3e', confirmButtonText: 'Yes, delete it!', reverseButtons: true }).then((r) => { if (r.isConfirmed) document.getElementById('delete-form-' + id).submit(); });
            }
        </script>
        @if(session('success'))
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8" x-data="{ showModal: false, editItem: null }">
        <div class="flex items-center justify-between py-5 lg:py-6">
            <div class="flex items-center space-x-4">
                <h2 class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl">Statuses</h2>
        ...
        <div class="card mt-3">
            ...
            <div class="flex flex-col space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4 sm:px-5">
                <form method="GET" action="{{ route('tickets/settings-statuses') }}" class="flex flex-1 flex-wrap items-center gap-3">
                    ...
                    @if(request()->hasAny(['search', 'status']))
                        <a href="{{ route('tickets/settings-statuses') }}" class="btn bg-slate-150 font-medium text-slate-800 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200/80 dark:bg-navy-500 dark:text-navy-50 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">Clear</a>
                    @endif
                </form>
            </div>
                ...
                <table class="is-hoverable w-full text-left">
                    <thead>
                        <tr>
                            ...
                            <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Closed</th>
                    ...
                    <tbody>
                        @forelse($statuses as $st)
                            <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-500 dark:text-navy-200 sm:px-5">{{ $statuses->firstItem() + $loop->index }}</td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="flex items-center space-x-2">
                                        <div class="size-3 rounded-full" style="background-color: {{ $st->color }}"></div>
                                        <span class="font-medium text-slate-700 dark:text-navy-100">{{ $st->name }}</span>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <code class="rounded bg-slate-150 px-1.5 py-0.5 text-xs text-slate-600 dark:bg-navy-500 dark:text-navy-200">{{ $st->code }}</code>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5"><div class="badge rounded-full px-2 py-0.5 text-xs text-white" style="background-color: {{ $st->color }}">{{ $st->color }}</div></td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">@if($st->is_closed) <span class="badge rounded-full bg-error/10 text-error text-xs">Yes</span> @else — @endif</td>
                                <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-500 dark:text-navy-200 sm:px-5">{{ $st->sort_order }}</td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="badge rounded-full {{ $st->status ? 'bg-success/10 text-success dark:bg-success/15' : 'bg-slate-200 text-slate-500 dark:bg-navy-500 dark:text-navy-200' }}">{{ $st->status ? 'Active' : 'Inactive' }}</div>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                    <div class="flex space-x-2">
                                        <button @click="showModal = true; editItem = { id: {{ $st->id }}, name: '{{ addslashes($st->name) }}', code: '{{ $st->code }}', color: '{{ $st->color }}', is_closed: {{ $st->is_closed ? 'true' : 'false' }}, sort_order: {{ $st->sort_order }}, status: {{ $st->status ? 'true' : 'false' }} }"
                                        ...
                                        <button @click="confirmDelete({{ $st->id }})" class="btn size-8 rounded-full p-0 hover:bg-error/20 focus:bg-error/20 text-error">
                                        ...
                                        <form id="delete-form-{{ $st->id }}" action="{{ route('tickets/settings-statuses-destroy', $st->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center">
                                    <div class="flex flex-col items-center space-y-2">
                                        ...
                                        <p class="text-slate-400 dark:text-navy-300">No statuses found</p>
                                        @if(request()->hasAny(['search', 'status']))
                                            <a href="{{ route('tickets/settings-statuses') }}" class="text-sm text-primary hover:text-primary-focus dark:text-accent-light dark:hover:text-accent">Clear filters</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="flex flex-col justify-between space-y-4 px-4 py-4 sm:flex-row sm:items-center sm:space-y-0 sm:px-5">
                <div class="flex items-center space-x-2 text-xs-plus">
                    <span>Showing</span>
                    <span class="font-semibold">{{ $statuses->firstItem() ?? 0 }} - {{ $statuses->lastItem() ?? 0 }}</span>
                    <span>of {{ $statuses->total() }} entries</span>
                </div>
                ...
                @if ($statuses->hasPages())
                    <ol class="pagination">
                        <li class="rounded-l-lg bg-slate-150 dark:bg-navy-500">
                            ...
                            @else
                                <a href="{{ $statuses->previousPageUrl() }}" class="flex size-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-300 focus:bg-slate-300 active:bg-slate-300/80 dark:text-navy-200 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" /></svg>
                                </a>
                            @endif
                        </li>
                        ...
                        <li class="rounded-r-lg bg-slate-150 dark:bg-navy-500">
                            @if ($statuses->hasMorePages())
                                <a href="{{ $statuses->nextPageUrl() }}" class="flex size-8 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-slate-300 focus:bg-slate-300 active:bg-slate-300/80 dark:text-navy-200 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90">
        ...
        <template x-teleport="body">
            <div x-show="showModal" x-transition.opacity class="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/60" @click.self="showModal = false">
                <div class="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-navy-700" @click.stop>
                    <h3 class="text-lg font-medium text-slate-700 dark:text-navy-100" x-text="editItem ? 'Edit Status' : 'Add Status'"></h3>

                    <form x-show="!editItem" method="POST" action="{{ route('tickets/settings-statuses-store') }}" class="mt-4 space-y-4">
                        @csrf
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="block"><span class="font-medium text-slate-600 dark:text-navy-100">Color</span><input class="mt-1.5 h-10 w-full cursor-pointer rounded-lg border border-slate-300" type="color" name="color" value="#3b82f6"></label>
                        ...
                        <div class="flex flex-wrap gap-6">
                            <label class="inline-flex items-center space-x-2"><input class="form-switch h-5 w-10 rounded-full bg-slate-300 before:rounded-full before:bg-white checked:bg-primary checked:before:bg-white dark:bg-navy-900 dark:before:bg-navy-300 dark:checked:bg-accent dark:checked:before:bg-white" type="checkbox" name="is_closed" value="1"><span class="text-sm text-slate-600 dark:text-navy-100">Closed State</span></label>
                    ...
                    <template x-if="editItem">
                        <form method="POST" :action="`/tickets/settings/statuses/${editItem.id}`" class="mt-4 space-y-4">
                            @csrf @method('PUT')
                            <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="flex flex-wrap gap-6">
                                <label class="inline-flex items-center space-x-2"><input class="form-switch h-5 w-10 rounded-full bg-slate-300 before:rounded-full before:bg-white checked:bg-primary checked:before:bg-white dark:bg-navy-900 dark:before:bg-navy-300 dark:checked:bg-accent dark:checked:before:bg-white" type="checkbox" name="is_closed" value="1" :checked="editItem.is_closed"><span class="text-sm text-slate-600 dark:text-navy-100">Closed State</span></label>
