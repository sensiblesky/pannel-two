                ...
                return {
                    ...
                    init() {
                        ...
                        this.rt = new RealtimeClient({
                            ...
                            pollMessagesUrl: '{{ route("tickets/poll-messages", $ticket->uuid) }}',
                            typingUrl: '{{ route("tickets/typing", $ticket->uuid) }}',
                            role: 'agent',
                        });
                    ...
                    async send() {
                        ...
                        try {
                            ...
                            const res = await fetch('{{ route("tickets/ajax-reply", $ticket->uuid) }}', {
                                method: 'POST',
                                headers: {
                    ...
                    async searchCanned() {
                        ...
                        try {
                            const resp = await fetch(`{{ route('tickets/canned-responses-search') }}?q=${encodeURIComponent(q)}`);
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8" x-data="{ showInfoPanel: false, activeTab: 'details' }">
        ...
        <div class="flex items-center justify-between py-5 lg:py-6">
            <div class="flex items-center space-x-4">
                <a href="{{ route('tickets/index') }}" class="btn size-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20">
        ...
        <form id="delete-ticket-form" action="{{ route('tickets/destroy', $ticket->uuid) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
            ...
            <div class="flex-1 space-y-4 sm:space-y-5 lg:space-y-6">
                ...
                <div class="card">
                    <div class="flex flex-wrap items-center gap-3 p-3 sm:px-5">
                        {{-- Status --}}
                        <form method="POST" action="{{ route('tickets/update-status', $ticket->uuid) }}" class="inline-flex">
                            @csrf @method('PUT')
                        ...
                        {{-- Priority --}}
                        <form method="POST" action="{{ route('tickets/update-priority', $ticket->uuid) }}" class="inline-flex">
                            @csrf @method('PUT')
                        ...
                        {{-- Assign --}}
                        <form method="POST" action="{{ route('tickets/assign', $ticket->uuid) }}" class="inline-flex">
                            @csrf @method('PUT')
                    ...
                    <div x-show="activeTab === 'details'" class="is-scrollbar-hidden space-y-4 overflow-y-auto p-4 sm:p-5" style="max-height: calc(100vh - 220px);">
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex flex-wrap gap-1.5">
                                @forelse($ticketTags as $tag)
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium" style="background-color: {{ $tag->color }}15; color: {{ $tag->color }}">
                                        {{ $tag->name }}
                                        <form method="POST" action="{{ route('tickets/remove-tag', [$ticket->uuid, $tag->id]) }}" class="ml-1 inline">
                                            @csrf @method('DELETE')
                            ...
                            @if($allTags->count() > $ticketTags->count())
                                <form method="POST" action="{{ route('tickets/add-tag', $ticket->uuid) }}" class="mt-2 flex items-center space-x-1">
                                    @csrf
                        ...
                        <div>
                            ...
                            <div class="mt-2 space-y-2">
                                @forelse($watchers as $watcher)
                                    <div class="flex items-center justify-between">
                                        ...
                                        <form method="POST" action="{{ route('tickets/remove-watcher', [$ticket->uuid, $watcher->user_id]) }}" class="inline">
                                            @csrf @method('DELETE')
                            ...
                            <form method="POST" action="{{ route('tickets/add-watcher', $ticket->uuid) }}" class="mt-2 flex items-center space-x-1">
                                @csrf
